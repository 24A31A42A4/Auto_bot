...
import { UserPlus, Mail, Lock, User, Phone, Hash, Zap, Loader2, AlertCircle, Globe, GraduationCap } from 'lucide-react'
...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (authData?.user) {
        const { error: profileError } = await supabase.from('Auto_bot').insert({
      ...
      <div className="relative z-10 fade-in auth-terminal-width">
        ...
        <div className="glass p-10 pt-20 rounded-[3.5rem] border-white/10 relative overflow-hidden shadow-2xl flex flex-col items-center">
          ...
          <div className="text-center mb-10">
            <h1 className="text-2xl font-black text-white mb-2 uppercase tracking-tighter">Create Account</h1>
          ...
          <form onSubmit={handleRegister} className="space-y-6 w-full relative z-10 text-left">
          ...
          <div className="mt-10 pt-8 border-t border-white/5 text-center w-full">
            <p className="text-gray-500 font-black text-[10px] uppercase tracking-widest">
              ...
              <Link to="/login" className="text-primary hover:text-white transition-colors ml-1">
                Sign In
              </Link>
            </p>
          </div>
